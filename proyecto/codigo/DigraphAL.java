...
/**
 * Lab4. Clase DigraphAL. Esta clase es una implementacion de un digrafo
 * usando listas de adyacencia. La plantilla de esta clase fue tomada del Github
 * del docente Mauricio Toro, en el siguiente enlace:
...
public class DigraphAL extends Digraph {
    ...
    /**
     * Constructor de la clase DigraphAL. Se crea y se inicializa la lista
     * enlazada de parejas.
     *
    ...
    public int getWeight(int source, int destination) {
        for (int i = 0; i < list.get(source).size(); i++) {
            if (list.get(source).get(i).vertice == destination) {
                return (int)(list.get(source).get(i).peso);
            }
        }
        return 0;
    }
